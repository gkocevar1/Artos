...
void refreshScreen()
{
  if (_lastDisplayRefreshTime == -1)
  {
    ...
    Serial.println("set start refresh time");
    return;  
  }
...
{
  Serial.print("resetServiceCounter:");
  Serial.println(key);
...
int getPressedButton()
{
  ...
  if (analogInput < 200)  //250 // 200
  {
    return btnUP;
  }
  
  if (analogInput < 300)  //450 // 300
  {
    return btnDOWN;
  }
  
  if (analogInput < 500)  //720  // 500
  {
    return btnLEFT; 
  }
  
  if (analogInput < 700)  //850 //700
  {
    return btnSELECT;  
  }
...
void displayOperationDetails(int operationTime, int fromServiceTime, long runnigTime, int quarters)
{
  ...
  printToSecondLine("");
  //TODO: this could be removed (or quarters only)		
  /*printToSecondLine("");
  lcd.setCursor(0, 1);
  lcd.print("T:");
  lcd.setCursor(2, 1);                
  
  lcd.print(runnigTime);
  lcd.setCursor(11, 1);
  lcd.print("Q:"); 
  lcd.setCursor(13, 1); 
  lcd.print(quarters);*/
